...
export async function initializeFarmingData(): Promise<void> {
  try {
    ...
    console.warn(
      `Failed to load farming database: ${error}. Make sure to run the fetch script first.`,
    );
 ...
 */
export function getCropDependencies(
  cropId: string,
): DependencyRelationship | undefined {
...
export function getCropsSupporting(cropId: string): string[] {
  ...
  Object.entries(farmingDatabase.dependencies.relationships).forEach(
    ([supporterCropId, deps]) => {
  ...
  Object.entries(farmingDatabase.dependencies.relationships).forEach(
    ([cropId, deps]) => {
      ...
      deps.directDependencies.forEach((depId) => {
        if (!allCropIds.has(depId) && depId !== "potato") {
          // Allow potato as it has no dependencies
